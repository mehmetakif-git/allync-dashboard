...
export default function GoogleCalendar() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'no_show': return 'bg-gray-500/10 text-muted border-secondary/30';
                ...
                <table className="w-full">
                  <thead className="bg-primary/50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Staff</th>
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {filteredAppointments.map((appointment) => (
                      <tr key={appointment.id} className="hover:bg-hover/50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            <p className="text-white font-medium">{appointment.customer_name}</p>
                            <p className="text-sm text-muted">{appointment.customer_phone}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <p className="text-white">{appointment.appointment_type}</p>
                          <p className="text-sm text-muted">{appointment.duration_minutes} min</p>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <p className="text-white">{appointment.requested_date}</p>
                          <p className="text-sm text-muted">{appointment.requested_time}</p>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-secondary">{appointment.source}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm text-muted">Monthly Bookings</span>
                </div>
                  ...
                  return (
                    <div key={type.name}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-secondary">{type.name}</span>
                        <span className="text-muted text-sm">{count} bookings ({percentage.toFixed(0)}%)</span>
                      </div>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6">
            ...
            <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-muted mb-2">Google Calendar ID</label>
                  <input type="text" defaultValue="techcorp@google.com" className="w-full px-4 py-3 bg-primary border border-secondary rounded-lg text-white cursor-not-allowed" readOnly />
                </div>
                <div>
                ...
                <div className="flex items-center justify-between p-4 bg-primary rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-muted">Automatically approve new appointment requests</p>
                  </div>
            ...
            <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
              ...
              <div className="space-y-3">
                {mockCalendarStaff.map((staff) => (
                  <div key={staff.id} className="flex items-center justify-between p-4 bg-primary rounded-lg">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      staff.is_active
                        ...
                        : 'bg-gray-500/10 text-muted border border-secondary/30'
                    }`}>
