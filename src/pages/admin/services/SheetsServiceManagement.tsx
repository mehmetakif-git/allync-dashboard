...
import { Edit3, Save, X, Building2, Sheet, TrendingUp, Users, BarChart3, Eye, ArrowLeft, Search } from 'lucide-react';

export default function SheetsServiceManagement() {
  ...
  const [serviceContent, setServiceContent] = useState({
    ...
    description_en: 'WhatsApp data queries from Google Sheets inventory/database. Query product information, check stock levels, and get pricing directly through WhatsApp.',
    description_tr: 'WhatsApp üzerinden Google Sheets envanter/veritabanı sorguları. Ürün bilgilerini sorgulayın, stok seviyelerini kontrol edin ve WhatsApp üzerinden fiyatlandırma bilgilerine erişin.',
    features_en: [
    ...
    features_tr: [
      'WhatsApp veri sorguları',
      'Gerçek zamanlı sheet senkronizasyonu',
      'Envanter yönetimi',
      'Ürün arama',
      'Fiyat ve stok kontrolleri',
      'Yapay zeka destekli arama',
      'Çoklu sheet desteği',
      'Özel sorgu şablonları',
    ],
    pricing: {
      starter: { price: 149, features: ['Up to 1,000 queries/month', 'Basic sheet sync', 'Email support'] },
      professional: { price: 299, features: ['Up to 10,000 queries/month', 'Advanced search', 'Multi-sheet support', 'Priority support'] },
      enterprise: { price: 599, features: ['Unlimited queries', 'Custom integrations', 'Dedicated support', 'Advanced analytics'] },
    },
  ...
  const globalStats = {
    ...
    totalQueries: companiesUsingService.reduce((sum, c) => sum + c.totalQueries, 0),
    totalSheets: companiesUsingService.reduce((sum, c) => sum + c.sheetsConnected, 0),
    avgResponseTime: '1.5 sec',
    monthlyRevenue: companiesUsingService.reduce((sum, c) => {
      const price = c.package === 'Starter' ? 149 : c.package === 'Professional' ? 299 : 599;
      return sum + price;
    }, 0),
  };
  ...
  const handleSaveContent = async () => {
    ...
    console.log('Sheets Service Content Updated:', serviceContent);
  };
        ...
        <div className="bg-gradient-to-br from-green-600 to-green-800 rounded-2xl p-8 text-white">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-3xl font-bold mb-2">Google Sheets Service</h1>
              <p className="text-green-100 text-lg">
                Manage Google Sheets integration service. Edit content, view company usage, and monitor analytics.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Google Sheets - Service Management</h2>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition-all whitespace-nowrap relative ${
                  activeTab === tab.id ? 'bg-green-600 text-white' : 'bg-secondary text-muted hover:bg-hover'
                }`}
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    disabled={!isEditing}
                    className={`w-full px-4 py-3 bg-primary border border-secondary rounded-lg text-white focus:outline-none focus:border-green-600 ${!isEditing && 'cursor-not-allowed opacity-70'}`}
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <textarea
                    ...
                    disabled={!isEditing}
                    rows={4}
                    className={`w-full px-4 py-3 bg-primary border border-secondary rounded-lg text-white focus:outline-none focus:border-green-600 resize-none ${!isEditing && 'cursor-not-allowed opacity-70'}`}
                  />
                </div>
                <div>
                  ...
                  <div className="space-y-2">
                    {serviceContent.features_en.map((feature, index) => (
                      <input
                        ...
                        disabled={!isEditing}
                        className={`w-full px-4 py-2 bg-primary border border-secondary rounded-lg text-white focus:outline-none focus:border-green-600 ${!isEditing && 'cursor-not-allowed opacity-70'}`}
                      />
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
                ...
                <p className="text-2xl font-bold text-white">{globalStats.totalQueries}</p>
              </div>
              <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
                <div className="flex items-center gap-3 mb-2">
                  <Sheet className="w-5 h-5 text-purple-500" />
                  <span className="text-sm text-muted">Sheets Connected</span>
                </div>
                <p className="text-2xl font-bold text-white">{globalStats.totalSheets}</p>
              </div>
              <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
                <div className="flex items-center gap-3 mb-2">
                ...
                <table className="w-full">
                  <thead className="bg-primary/50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Queries</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-muted uppercase tracking-wider">Sheets</th>
                  ...
                  {companiesUsingService.map((company) => (
                    <tr key={company.id} className="hover:bg-hover/50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-secondary">{company.totalQueries}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-secondary">{company.sheetsConnected}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-secondary">{company.avgResponseTime}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button onClick={() => window.location.hash = `company-detail/${company.id}`} className="p-2 hover:bg-green-500/10 rounded-lg transition-colors">
        ...
        {activeTab === 'analytics' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
                <p className="text-sm text-muted mb-1">Total Queries</p>
                <p className="text-3xl font-bold text-white">{globalStats.totalQueries}</p>
              </div>
              <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
                <p className="text-sm text-muted mb-1">Sheets Connected</p>
                <p className="text-3xl font-bold text-white">{globalStats.totalSheets}</p>
              </div>
              <div className="bg-card backdrop-blur-sm border border-secondary rounded-xl p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-3">
                    <div className="bg-gradient-to-r from-green-600 to-green-800 h-3 rounded-full" style={{ width: '50%' }}></div>
                  </div>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-2">
