...
import { Globe, ArrowLeft, Loader2, Upload, Image as ImageIcon, Trash2, X, Calendar, FileText } from 'lucide-react';
...
export default function WebsiteServiceManagement() {
  ...
  const loadServiceData = async () => {
    try {
      ...
      if (!websiteService) {
        console.error('Website service not found');
        return;
      }

      // Fetch companies using this service with their custom pricing
      const companiesData = await getAllCompaniesWithServicePricing(websiteService.id);
